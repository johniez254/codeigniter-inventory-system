...
<div class="success-mess"></div>
                                     <!--/success-messages-->
                                        ...
                                        <form class="form-horizontal" method="POST" action="<?php echo base_url().'manager/stock/updatearraystock' ?>" id="updateForm">
...
?>
                                        <tr id="row<?php echo $p; ?>" class="<?php echo $arrar; ?>">
                                            ...
                                            <td><input readonly type="number" name="bprice[]" min="1" value="<?=$b_price?>" autocomplete="off" class="form-control" placeholder="Buying price" id="bprice<?php echo $p;?>"></td>
                                            <td><input readonly type="number" min="1" name="sprice[]" value="<?=$s_price?>" autocomplete="off" class="form-control" placeholder="Selling price" id="sprice<?php echo $p;?>"></td>
                                            <td><input type="number" min="1" name="available[]" value="<?=$available?>" autocomplete="off" class="form-control" id="available<?php echo $p;?>">
                                            <input type="hidden" min="1" name="av[]" value="<?=$available?>" autocomplete="off" class="form-control" id="av<?php echo $p;?>">
                                            </td>
                                            <td><input type="number" name="damaged[]" class="form-control" value="<?= $damaged ?>" id="damaged<?php echo $p; ?>" onkeyup="changeAvailable(<?php echo $p ?>)"><input type="hidden" min="1" name="dam[]" value="<?=$damaged?>" autocomplete="off" class="form-control" id="dam<?php echo $p;?>">                                     </td>
                                        </tr>
                                        <?php 
										$arrar++;
												$p++;
...
<script>
function changeAvailable(row=null) {

	if(row) {
		var avail = Number($("#available"+row).val());
		var av = Number($("#av"+row).val());
		var damaged =  Number($("#damaged"+row).val());
		var dam =  Number($("#dam"+row).val());
		
		var available = Number($("#available"+row).val()) - Number($("#damaged"+row).val());
		available = available.toFixed(2);
		$("#available"+row).val(available);
		
		if (damaged > avail) {
			$("#damaged"+row).val(dam);
			//$("#available"+row).val(av);
			var available = Number($("#av"+row).val()) - Number($("#damaged"+row).val());
			available = available.toFixed(2);
			$("#available"+row).val(available);
			...
			$("#damaged"+row).after('<small class="text-danger"> <b>Max Damage : '+av+' </b></small>');
